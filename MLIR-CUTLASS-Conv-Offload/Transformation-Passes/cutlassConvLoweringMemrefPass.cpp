...
struct ConvToCutlassPattern : public OpRewritePattern<linalg::Conv2DNchwFchwOp> {
  ...
  LogicalResult matchAndRewrite(linalg::Conv2DNchwFchwOp convOp,
    ...
    Value outputCasted = rewriter.create<memref::CastOp>(
        loc, UnrankedMemRefType::get(rewriter.getF32Type(), 0), output);
    Value inputCasted = rewriter.create<memref::CastOp>(
        loc, UnrankedMemRefType::get(rewriter.getF32Type(), 0), input);
    Value filterCasted = rewriter.create<memref::CastOp>(
...
private:
  func::FuncOp getOrCreateCutlassConvKernel(ModuleOp module,
    ...
    Type memRefType = UnrankedMemRefType::get(rewriter.getF32Type(), 0);
    ...
    auto funcType = rewriter.getFunctionType(inputTypes, ArrayRef<Type>{});
